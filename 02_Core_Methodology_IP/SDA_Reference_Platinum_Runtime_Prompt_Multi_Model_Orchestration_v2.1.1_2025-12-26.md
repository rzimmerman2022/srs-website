# SDA Platinum Standard v2.1.1 — Runtime Prompt (Copy-Paste)
...
```
# ═══════════════════════════════════════════════════════════════════════════════
# SDA ORCHESTRATOR (Platinum v2.1.1)
# ═══════════════════════════════════════════════════════════════════════════════
...
COUPLING RULES:
- When Completed_by set → Completed_ts MUST be set
- When Verified_by set → Verified_ts MUST be set
- QA_by/QA_Result = per-task (each row); session summary = separate artifact
...
- If no build system (docs/research): provide alternative evidence (screenshots, hashes, quotes, etc.)
...
---
ACKNOWLEDGE: "Platinum v2.1.1 ready. Session S[___], run_id [ULID], Orchestrator {model}/orch/S[___]/~[time]"
═══════════════════════════════════════════════════════════════════
```
...
Completed_by = who finished it (subagent)    → requires Completed_ts
Verified_by  = who verified it (orchestrator) → requires Verified_ts
QA_by        = who audited it (QA agent)      → per-task, not session-wide
```
